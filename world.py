...
class World:
    ...
    def get_distance_matrix(self, adjacency_matrix):
        """
    ...
    def save_adjacency_plot(self, A, cmap='binary', alpha=0.3):
        plt.imshow(A, cmap=cmap,extent=(0,self.n_agents+self.checkpoints_coord.shape[0],self.n_agents+self.checkpoints_coord.shape[0],0))
        plt.colorbar()
        plt.axhspan(xmin=0, xmax=((self.n_agents)/(self.n_agents+self.checkpoints_coord.shape[0])), ymin=0, ymax=((self.n_agents)), alpha=alpha, facecolor='g')
        plt.axhspan(xmin=((self.n_agents)/(self.n_agents+self.checkpoints_coord.shape[0])), xmax=1, ymin=((self.n_agents)), ymax=self.n_agents+self.checkpoints_coord.shape[0], alpha=alpha, facecolor='r')
        plt.legend(handles=[Patch(facecolor='g', alpha=0.3, label='Edges between agents'),Patch(facecolor='r', alpha=0.3, label='Edges between checkpoints')], bbox_to_anchor=(0, 1.16), loc='upper left')
    ...
    def save_distance_plot(self, D, cmap='copper', boundaries=None, cluster_labels=None):
        if boundaries=='agents_and_checkpoints':
            ...
            plt.colorbar()
            plt.axhspan(xmin=0, xmax=((self.n_agents)/(self.n_agents+self.checkpoints_coord.shape[0])), ymin=0, ymax=((self.n_agents)), facecolor='none', edgecolor='g', linewidth=2)
            plt.axhspan(xmin=((self.n_agents)/(self.n_agents+self.checkpoints_coord.shape[0])), xmax=1, ymin=((self.n_agents)), ymax=self.n_agents+self.checkpoints_coord.shape[0], facecolor='none', edgecolor='r', linewidth=2)
            plt.legend(handles=[Patch(facecolor='none',edgecolor='g', label='Shortest path between agents', linewidth=2),Patch(facecolor='none',edgecolor='r', label='Shortest path between checkpoints', linewidth=2)], bbox_to_anchor=(0, 1.16), loc='upper left')
            ...
            for i in np.unique(cluster_labels):
                ...
                plt.axhspan(xmin=interm_count/(D.shape[0]), xmax=count/(D.shape[0]), ymin=interm_count, ymax=count, facecolor='none', edgecolor='r', linewidth=2.5)
    ...
    def purge_checkpoints(self, reached_radius, assignment_group=None , purge_list=None):
        ...
        self.remaining_checkpoints = [checkpoint for checkpoint in self.remaining_checkpoints if checkpoint not in purge_list]
        return purge_list

    def get_CSE(self, D, dim):
        """
        ...
        try:n_zeros = np.max(np.where(e < 1e-3))
        except:n_zeros=-1
